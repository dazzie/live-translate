...
| **Node gateway** | ✅ **Built** (2 TODOs done) — logging + trace id, AI-service proxy, streaming passthrough | `backend/gateway-node/` |
| **Python AI service** | ✅ **Built** — LLM (Anthropic/OpenRouter), two-tier cache, SSE streaming | `backend/ai-service-python/` |
...
- [x] **LLM** — translate EN → **Mexican Spanish** (es-MX register, not generic Spanish) via a real LLM call.
...
- [x] **Deployed on Fly.io** — both services deployed; the extension works against the public gateway URL, not just localhost.
- [x] **Contract** — every endpoint matches the shapes above; the provided widget works unmodified.
- [x] **Product evaluation** — `PRODUCT_EVAL.md` generated; live-website test passed on real sites (`homedepot.com`, `rte.ie/news`); 60–90s video demo attached (https://youtu.be/nxAdKuOFKWg).
...
**LLM**
- [x] Output is natural **Mexican Spanish (es-MX)**, translation only — no preamble, no wrapping quotes
...
# 2) Node gateway  → http://localhost:8787
cd backend/gateway-node
npm install
...
Both services run as separate Fly apps (Docker builds; `fly.toml` in each service dir):

| Service | App | URL |
|---------|-----|-----|
| Node gateway (browser-facing) | `fde-lt-gw-dm` | https://fde-lt-gw-dm.fly.dev |
| Python AI service | `fde-lt-ai-dm` | https://fde-lt-ai-dm.fly.dev |
...
fly apps create fde-lt-gw-dm
fly secrets set AI_SERVICE_URL=https://fde-lt-ai-dm.fly.dev --app fde-lt-gw-dm
fly deploy . --app fde-lt-gw-dm \
  --config backend/gateway-node/fly.toml \
  --dockerfile backend/gateway-node/Dockerfile
```

Point the **extension popup's backend URL** at the public gateway
(`https://fde-lt-gw-dm.fly.dev`) for the live-website test.

> Notes: each app runs a single machine so the two-tier cache stays coherent (the
> memory + SQLite tiers are per-machine). The cache is not shared across machines,
> and on Fly the SQLite file is ephemeral unless a volume is mounted — mounting a
> volume at a `TRANSLATION_DB_PATH` is the hardening step for cross-restart
> persistence in production. Keeping the AI service private via `flycast` (so only
> the gateway can reach it) is a further hardening step.
